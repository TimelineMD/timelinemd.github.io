...
function initSlogans() {
  ...
  const slogans = {
    ru: [
      "Молдова и мир: история на одной линии",
      "От Древнего Рима до независимой Молдовы",
      "История мира через призму Молдовы",
      "Понять мир — понять историю Молдовы",
      "От первых цивилизаций до наших дней"
    ],
    ro: [
      "Moldova și lumea pe o singură axă",
      "De la Roma antică la Moldova modernă",
      "Istoria lumii prin prisma Moldovei",
      "Înțelege lumea, înțelege istoria Moldovei",
      "De la primele civilizații până azi"
    ]
  };
  ...
  function getCurrentLang() {
    ...
    return (attr === 'ro' || attr === 'ru') ? attr : 'ru';
  }
  ...
  function applySlogan(immediate) {
    const lang = getCurrentLang();
    if (lang !== currentLang) {
      currentLang = lang;
      currentText = "";
    }
    const list = slogans[lang] || slogans.ru;
    const next = pickRandom(list, currentText);
    currentText = next;
  ...
  // первый слоган сразу
  applySlogan(true);
  // и дальше каждые 5 секунд
  setInterval(applySlogan, 5000);
}

const TOC_SCROLL_KEY = "timeline_toc_scroll";

function setupTocScrollMemory() {
  ...
  if (isIndex) {
    // для главной всегда открываем оглавление с начала
    localStorage.removeItem(TOC_SCROLL_KEY);
    toc.scrollTop = 0;
  } else {
    const saved = localStorage.getItem(TOC_SCROLL_KEY);
    if (saved !== null) {
  ...
  toc.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      saveRequested = false;
      try {
        localStorage.setItem(TOC_SCROLL_KEY, String(toc.scrollTop));
      } catch (e) {
        // ignore
      }
    });
